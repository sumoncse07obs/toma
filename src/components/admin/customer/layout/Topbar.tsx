...
type Props = {
  user?: User | null;
  title?: string;                 // ✅ NEW: allow a custom title
  collapsed: boolean;
...
export default function Topbar({
  user: propUser,
  title = "Admin Dashboard",      // ✅ default if none provided
  collapsed,
  setCollapsed,
  setSidebarOpen,
  ...
  return (
    <header className="h-16 px-4 lg:px-6 border-b bg-white flex items-center justify-between">
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-semibold text-slate-900">{title}</h1> {/* ✅ uses dynamic title */}
      </div>
