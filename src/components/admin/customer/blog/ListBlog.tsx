// src/components/admin/customer/blog/ListBlog.tsx
...
// --- NEW: extract :customerId from pathname like /admin/customer-dashboard/7/...
...
function RowSkeleton() {
  return (
    <tr className="border-t animate-pulse">
      <td className="px-3 py-3"><div className="h-3 w-64 bg-gray-200 rounded" /></td>
      <td className="px-3 py-3"><div className="h-3 w-72 bg-gray-200 rounded" /></td>
      <td className="px-3 py-3"><div className="h-5 w-20 bg-gray-200 rounded-full" /></td>
      <td className="px-3 py-3"><div className="h-3 w-28 bg-gray-200 rounded" /></td>
      <td className="px-3 py-3"><div className="h-8 w-16 bg-gray-200 rounded ml-auto" /></td>
    </tr>
  );
}

export default function ListBlog({ customerId, perPage = 10 }: Props) {
  const [user, setUser] = React.useState<User | null>(null);

  // --- NEW: URL param + pathname access
  ...
  // --- UPDATED: prefer URL id (param or regex) before fallbacks/user/me
  ...
  }, [customerIdParam, location.pathname, customerIdFromAPI, customerId, user]);
  ...
  // --- UPDATED: resolve customer id only if URL didn’t provide one
  React.useEffect(() => {
    if (!isAuthed()) {
    ...
    async function tryResolve() {
      ...
      // If we already have a valid id from URL/user/props, stop.
      if (validCustomerId) {
  ...
  const adminNewUrl = validCustomerId
    ? `/admin/customer-dashboard/${validCustomerId}/blog/new`
    : `/customer/blog/new`;

  const adminLogsUrl = validCustomerId
    ? `/admin/customer-dashboard/${validCustomerId}/blog/logs`
    : `/customer/logs/blog/`;
                  ...
                  <tr key={r.id} className="border-t">
                    ...
                    <td className="px-3 py-2">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          to={`/admin/customer-dashboard/${r.customer_id}/${CONTEXT}/view/${r.id}`}
