// src/components/admin/customer/PublishPost.tsx
...
export default function PublishPost() {
  // NOTE: admin route: /admin/customer-dashboard/:customerId/blog/post/:id
  const { customerId: customerIdParam, id } = useParams<{ customerId?: string; id: string }>();
  const navigate = useNavigate();
  const { pathname } = useLocation();

  const isAdminRoute = useMemo(() => pathname.toLowerCase().includes("/admin/customer-dashboard/"), [pathname]);
  const isCustomerRoute = useMemo(() => pathname.toLowerCase().includes("/customer/"), [pathname]);
  ...
  // ====== figure out customerId source ======
  const [customerId, setCustomerId] = useState<number | null>(null);

  useEffect(() => {
    // If we are on admin route, prefer customerId from URL (e.g., /admin/customer-dashboard/7/...)
    if (isAdminRoute && customerIdParam) {
    ...
    // Otherwise fallback to /customers/me
    (async () => {
      try {
  ...
  }, [isAdminRoute, customerIdParam]);

  // ====== dynamic links (Back / Logs / Convert) ======
  const contentId = record?.id ?? (id ? parseInt(id, 10) : null);

  const adminRoot = useMemo(() => {
    const cid = customerIdParam || (customerId != null ? String(customerId) : "");
    return `/admin/customer-dashboard/${cid}`;
  }, [customerIdParam, customerId]);

  const backUrl = useMemo(() => {
    if (contentId == null) return isAdminRoute ? adminRoot : "/customer";
    return isAdminRoute
      ? `${adminRoot}/${context}/view/${contentId}`
      : `/customer/${context}/view/${contentId}`;
  }, [isAdminRoute, adminRoot, context, contentId]);

  const logsUrl = useMemo(() => {
    if (contentId == null) return "#";
    return isAdminRoute
      ? `${adminRoot}/${context}/log/${contentId}`
      : `/customer/${context}/log/${contentId}`;
  }, [isAdminRoute, adminRoot, context, contentId]);
  ...
  const convertHref = useMemo(() => {
    if (!genId) return "#";
    return isAdminRoute
      ? `${adminRoot}/${promptFor}/posttoblotato/${genId}`
      : `/customer/${promptFor}/posttoblotato/${genId}`;
  }, [isAdminRoute, adminRoot, promptFor, genId]);
  ...
  useEffect(() => {
    ...
    if (customerId != null) loadSettings(customerId);
    return () => {
      cancelled = true;
    };
  }, [customerId]);
  ...
  async function approve(scheduledAtIso?: string) {
    ...
    if (!customerId) {
      ...
      setApproveError("Missing customer_id.");
      return;
    }
    ...
    const body: any = {
      ...
      customer_id: customerId, // ✅ now correct for admin/customer routes
      platform: platformKey,
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center py-8 px-4">
      ...
      {/* Hint for non-Blotato media on video */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-6xl">
        ...
        <div className="space-y-2">
          {/* ...left column unchanged except data-driven visibility... */}
          {(() => {
            if (Object.keys(FIELD_MAP).length === 0) return null;
        ...
        <div className="md:col-span-2 border border-gray-300 rounded-lg p-4">
          ...
          {(() => {
            ...
            if (media.showVideo) {
              return isPlayableVideo(media.videoUrl) ? (
                <div
                ...
                >
                  <video key={media.videoUrl} src={media.videoUrl} controls className="w-full h-full object-cover" />
                </div>
              ) : (
                <div
            ...
            if (media.showImage) {
              return media.imageUrl ? (
                <div className={`rounded-md mb-3 border border-gray-200 overflow-hidden bg-gray-50 ${mediaBoxClassBySize[previewSize]}`}>
                  <img src={media.imageUrl} alt="Post image" className="w-full h-full object-cover" />
                </div>
              ) : (
                <div
                  className={`rounded-md mb-3 border-2 border-dashed border-gray-300 flex items-center justify-center text-gray-400 text-sm ${mediaBoxClassBySize[previewSize]}`}
