...
function CreateUserModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  const [form, setForm] = React.useState({ name: "", email: "", password: "", role: "user" as "admin" | "user" | "customer" });
  ...
  async function submit() {
    ...
    try {
      ...
      try {
        ...
        if (j?.errors) setErrors(Object.fromEntries(Object.entries(j.errors).map(([k, v]: any) => [k, (v?.[0] as string) || ""])));
      ...
      <div className="fixed inset-0 z-50 grid place-items-center p-4">
        <Card>
          ...
          <div className="mt-6 flex items-center justify-end gap-2">
            ...
            <PrimaryBtn onClick={submit} disabled={busy}>Create</PrimaryBtn>
          </div>
        </Card>
      </div>
    </>
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 grid place-items-center p-4">
        <Card>
          ...
          <div className="mt-6 flex items-center justify-end gap-2">
            <GhostBtn onClick={onClose}>Cancel</GhostBtn>
            <PrimaryBtn onClick={submit} disabled={busy}>Save</PrimaryBtn>
          </div>
        </Card>
      </div>
    </>
      ...
      <div className="fixed inset-0 z-50 grid place-items-center p-4">
        <Card>
          ...
          <div className="mt-6 flex items-center justify-end gap-2">
            ...
            <PrimaryBtn onClick={submit} disabled={busy || !password}>Update</PrimaryBtn>
          </div>
        </Card>
      </div>
    </>
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow p-4 grid grid-cols-1 md:grid-cols-4 gap-3">
        <div className="md:col-span-2">
          <Field label="Search">
            <TextInput value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="name, email, role…" />
          </Field>
        </div>
        <div>
          <Field label="Role">
                ...
                <tr key={u.id} className="border-t">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-2">
                      ...
                      <GhostBtn className="text-red-600" onClick={() => remove(u)}>Delete</GhostBtn>
                    </div>
                  </td>
                </tr>
              ))
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <GhostBtn onClick={() => setPage(1)} disabled={page <= 1}>« First</GhostBtn>
          <GhostBtn onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1}>‹ Prev</GhostBtn>
          <span className="text-sm">Page {page} / {lastPage}</span>
          <GhostBtn onClick={() => setPage((p) => Math.min(lastPage, p + 1))} disabled={page >= lastPage}>Next ›</GhostBtn>
          <GhostBtn onClick={() => setPage(lastPage)} disabled={page >= lastPage}>Last »</GhostBtn>
        </div>
      </div>

      {showCreate && <CreateUserModal onClose={() => setShowCreate(false)} onCreated={() => { load(); loadStats(); }} />}
      {editUser && <EditUserModal user={editUser} onClose={() => setEditUser(null)} onUpdated={() => { load(); loadStats(); }} />}
